...
パイプラインを作成したら、パイプラインを開始できる REST エンドポイントを公開できます。これにより、パイプラインをオンデマンドまたはスケジュールされた時刻に実行できます。

## 開始する前に

このラボを開始する前に、このラボで使用する Azure Machine Learning ワークスペースと他のリソースを作成するタスクを含む[ラボ 1A](Lab01A.md) と[ラボ 1B](Lab01B.md)、およびこのラボで公開するパイプラインを作成する[ラボ 6A](Lab06A.md) を完了させてください。
...
3. Jupyter のホーム ページの**Users/DP100** フォルダーで、**06B - Publishing a Pipeline.ipynb** Notebook を開きます。次に、Notebook 内の注意事項を読み、各コード セルを順番に実行します。
